...
* Get the **face count** of a FR.
  ```bash
  curl --request GET \
  --url 'http://192.168.1.165:80/ISAPI/Intelligent/FDLib/Count?format=json' \
...
* Get the **card count** of a FR
  ```bash
  curl --request GET \
  --url 'http://192.168.1.165:80/ISAPI/AccessControl/CardInfo/Count?format=json' \
  --header 'Accepts: application/json' \
  --header 'Content-Type: application/json' \
  -u "admin:Hik12345" \
  --digest
...
### Frequently Used Ones
  1. Delete Users from FR.
     ...
     **For HAC (...175) - Hikvision**
     ...
     ```
  2. Get user count (Not face count in the device) - **Entry FR(...165) - Hikvision**
     ```bash
...
Tested model - **DHI-ASI7213X-T1**

* List the available card records (users are inserted as cards. This not guarenteed faces available in FR.)
  ```bash
  curl --digest -u admin:Hik12345 \
    "http://192.168.1.73/cgi-bin/recordFinder.cgi?action=find&name=AccessControlCard"
  ```
